...
export const ProgramwiseScenario: React.FC = () => {
        ...
        try {
            ...
            if (data) {
                ...
                data.forEach((item: any) => {
                    const scheme = item.scheme_name || 'Unknown';
                    const current = map.get(scheme) || {
                        scheme_name: scheme,
                        female_trained: 0,
                        male_trained: 0
                    };
    ...
    const handleExport = () => {
        ...
        link.href = url;
        link.download = `Programwise_As_Is_${currentDistrict}.csv`;
        link.click();
    };
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex justify-between items-center">
                <div>
                    <h2 className="text-xl font-bold text-gray-900">Program wise As Is Scenario</h2>
                    <p className="text-sm text-gray-500">By Scheme - Reference Data from Trainee Analysis</p>
                </div>
            ...
            {showVisuals && rows.length > 0 && (
                <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm animate-in fade-in slide-in-from-top-4 duration-500 mb-6">
                    <h3 className="text-sm font-bold text-gray-700 mb-4">Trained Candidates by Scheme (Gender Split)</h3>
                    <div className="h-[450px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            ...
                            >
                                ...
                                <XAxis dataKey="scheme_name" angle={-45} textAnchor="end" interval={0} height={100} />
                                <YAxis />
                                <Tooltip />
                ...
                <table className="w-full text-sm text-center">
                    <thead className="bg-gray-100 text-gray-700 font-bold uppercase text-xs">
                        <tr>
                            ...
                            <th className="px-6 py-4 text-left border-r">Program / Scheme</th>
                    ...
                    <tbody className="divide-y divide-gray-200">
                        {rows.map((row, index) => {
                            ...
                            return (
                                <tr key={row.scheme_name} className="hover:bg-gray-50">
                                    <td className="px-6 py-4 border-r text-gray-500">{index + 1}</td>
                                    <td className="px-6 py-4 text-left font-medium text-gray-900 border-r">{row.scheme_name}</td>
                        ...
                        {rows.length === 0 && (
                            <tr>
                                <td colSpan={5} className="px-6 py-8 text-center text-gray-500">
                                    No scheme data found in Trainee Analysis.
                                </td>
                            </tr>
                        )}
                    </tbody>
