...
export const PopulationDataEntry: React.FC<PopulationDataEntryProps> = ({ sourceType, title, subtitle }) => {
    ...
    const chartData = useMemo(() => {
        const parse = (val: string) => Number(val) || 0;

        // 1. Age Group Data
        const ageGroupRows = rows.filter(r => r.group_name === 'Population');
        const ageGroupData = ageGroupRows.map(r => ({
            name: r.row_label,
            Population: parse(r.total_total),
            Male: parse(rows.find(m => m.group_name === 'Male' && m.row_label === r.row_label)?.total_total || '0'),
            Female: parse(rows.find(f => f.group_name === 'Female' && f.row_label === r.row_label)?.total_total || '0')
        }));

        // 2. Rural vs Urban (District Total)
        // Adjust looking for 'Dakshina Kannada' or relevant district. defaulting to the last 'Main' row which is usually the district
        const mainRows = rows.filter(r => r.group_name === 'Main');
        const districtRow = mainRows[mainRows.length - 1]; // Assumption: Last main row is the target district
        const ruralUrbanData = districtRow ? [
            { name: 'Rural', value: parse(districtRow.total_rural) },
            { name: 'Urban', value: parse(districtRow.total_urban) }
        ] : [];

        // 3. Social Category Split (District Total)
        const socialData = districtRow ? [
            { name: 'General', value: parse(districtRow.total_total) - parse(districtRow.sc_total) - parse(districtRow.st_total) },
            { name: 'SC', value: parse(districtRow.sc_total) },
            { name: 'ST', value: parse(districtRow.st_total) }
        ] : [];

        return { ageGroupData, ruralUrbanData, socialData };
    }, [rows]);
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            {showVisuals && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-top-4 duration-500 mb-8">
                    {/* Chart 1: Age Group Distribution */}
                    <div className="lg:col-span-2 bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                        <h3 className="text-sm font-bold text-gray-700 mb-4">Population by Age Group (Youth)</h3>
                        <div className="h-[300px] w-full">
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={chartData.ageGroupData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                    ...
                    <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                        <h3 className="text-sm font-bold text-gray-700 mb-4">Rural vs Urban Split</h3>
                        <div className="h-[300px] w-full">
                            <ResponsiveContainer width="100%" height="100%">
                                <PieChart>
                                    <Pie
                                        data={chartData.ruralUrbanData}
                                        cx="50%"
                                        cy="50%"
                                        innerRadius={60}
                                        outerRadius={80}
                                        ...
                                        dataKey="value"
                                    >
                                        {chartData.ruralUrbanData.map((_, index) => (
                    ...
                    {/* Chart 3: Social Category */}
                    <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                        <h3 className="text-sm font-bold text-gray-700 mb-4">Social Category Composition</h3>
                        <div className="h-[300px] w-full">
                            <ResponsiveContainer width="100%" height="100%">
                                <PieChart>
                                    <Pie
