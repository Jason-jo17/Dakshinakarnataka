  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      ...
      if (error) {
        ...
        // Auto-generate credentials for the new company
        try {
          const empName = inserted[0].employer_name;
          const sanitizedName = empName.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
          await useCredentialStore.getState().generateCredential({
            role: 'company',
            entityId: inserted[0].id, // Link to the specific record ID
            entityName: empName,
            username: `company_${sanitizedName}`,
            email: inserted[0].contact_person_email,
            // Password will be auto-generated by the store if not provided
          });
          alert('Employer added and credentials generated successfully!');
        } catch (credError) {
          console.error("Error generating credentials:", credError);
          alert('Employer added, but failed to generate credentials. Please use the Credential Manager to sync.');
        }
      }
    }
  };
