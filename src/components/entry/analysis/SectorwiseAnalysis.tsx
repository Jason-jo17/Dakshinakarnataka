...
export const SectorwiseAnalysis: React.FC = () => {
  ...
  // Form State
  const [newEntry, setNewEntry] = useState<Omit<SectorData, 'id'>>({
    sector_name: '',
    male_trained: 0,
    male_placed: 0,
  ...
  const handleFormChange = (field: keyof Omit<SectorData, 'id'>, value: string) => {
    const numValue = field === 'sector_name' ? value : (value === '' ? 0 : Number(value));
  ...
  const handleInputChange = (index: number, field: keyof SectorData, value: string) => {
    const newData = [...data];
  ...
  const getRowTotals = (row: SectorData) => {
    return {
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = data.map(row => ({
        id: row.id, // Include ID if updating existing
      ...
      if (savedData) {
        // Re-fetch to ensure sync or map IDs back
        fetchData();
      }
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', `Sectorwise_Analysis_${currentDistrict}_${year}.csv`);
  ...
  const handleImportCSV = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        ...
        // Logic: Import appends new sectors or updates existing ones by name
        // Ideally we replace the list or merge. Let's merge based on sector name.

        const mergedData = [...data]; // Start with current data
        ...
        parsedData.forEach((row: any) => {
          ...
          const existingIndex = mergedData.findIndex(d => d.sector_name.toLowerCase() === sectorName.toLowerCase());
  ...
  const chartData = data.map(d => ({
    name: d.sector_name.length > 15 ? d.sector_name.substring(0, 15) + '...' : d.sector_name,
    full_name: d.sector_name,
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          <h2 className="text-xl font-bold text-gray-900">Table D: Sectorwise Analysis</h2>
      ...
      {showVisuals && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8 animate-in fade-in slide-in-from-top-4 duration-500">
          <AnalysisVisuals
            title="Top Sectors: Trained vs Placed"
            data={chartData}
            visualsType="bar"
            xAxisKey="name"
            barKeys={[
          ...
          />
          <AnalysisVisuals
            title="Trained Distribution by Sector"
            data={chartData}
            visualsType="pie"
            pieKey="trained"
            pieNameKey="name"
      ...
      <div className="mb-8 bg-slate-50 p-6 rounded-xl border border-slate-200">
        <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wider mb-4">Add New Sector Entry</h3>

        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
          <div className="md:col-span-4">
            <label className="block text-xs font-semibold text-gray-500 uppercase mb-1">Sector Name</label>
            <input
              type="text"
              ...
              {data.length === 0 && (
                <tr>
                  <td colSpan={14} className="px-6 py-12 text-center text-gray-500 italic">
                    No sector data added yet. Use the form above to add a new sector.
                  </td>
                </tr>
              )}
            </tbody>
